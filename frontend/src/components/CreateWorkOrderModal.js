  ...
  const handleSubmit = async () => {
    ...
    const validated = orders.map((o) => {
      ...
      if (!o.wo_no || !o.wo_name || !o.wo_start_date || !o.wo_finish_date || !o.wo_staff) {
        ...
        if (woEnd < woStart) {
          error = returnTitle('create_wo.invalid_date_range');
        } else if (woStart < partStart || woEnd > partEnd) {
          error = returnTitle('create_wo.outside_part_dates');
        }
      }
      return { ...o, error };
    });
    ...
    try {
      ...
      setAlert({ show: true, variant: 'success', message: returnTitle('create_wo.created_successfully') });
      setTimeout(() => {
      ...
      setAlert({ show: true, variant: 'danger', message: returnTitle('create_wo.creation_failed') });
    } finally {
      setSubmitting(false);
    }
  };
          ...
          {orders.map((order, index) => (
            <div key={index} className="border rounded p-3 mb-3 bg-dark bg-opacity-50">
              <Row className="align-items-end g-3">
                ...
                <Col md={2}>
                  <Form.Label>{returnTitle('create_wo.wo_start_date')}</Form.Label>
                  <Form.Control
                    type="date"
                ...
                <Col md={2}>
                  <Form.Label>{returnTitle('create_wo.wo_finish_date')}</Form.Label>
                  <Form.Control
                    type="date"
