...
export default function CreatePositionModal({ show, onHide, department, onCreated }) {
  ...
  useEffect(() => {
    if (show && department) {
      axiosInstance.get(`/job-positions/department/${department.department_id}/`)
        ...
        axiosInstance.post('/job-positions/', {
            position_name: name,
            ...
            .then(() => {
            setSuccess(returnTitle('add_depart.successfully_created'));
  ...
  return (
    <Modal show={show} onHide={onHide} centered dialogClassName="custom-fin-modal">
      <Modal.Header closeButton style={{ backgroundColor: '#2a3b4d', color: 'white' }}>
        <Modal.Title>{returnTitle('add_depart.add_position')}</Modal.Title>
      </Modal.Header>
      ...
      <Modal.Footer style={{ backgroundColor: '#2a3b4d' }}>
        <Button variant="secondary" onClick={onHide}>
          {returnTitle('app.cancel')}
            ...
            <Button variant="success" onClick={handleSubmit} disabled={submitting || lockUI}>
                {submitting ? (
                    <>
