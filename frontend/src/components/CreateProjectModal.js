...
export default function CreateProjectModal({ show, onHide, onCreated }) {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      currency: form.currency.value, // ✅ include currency
    };
    ...
    try {
      ...
      setTimeout(() => {
        ...
        console.log('e:',err)
        setAlertMsg('❌ ' + 'Project creation failed');
        setSubmitting(false);
      }
  };
    ...
    <Modal show={show} onHide={onHide} centered size="lg" backdrop="static" dialogClassName="custom-fin-modal">
      <Modal.Body className="p-4 text-light">
        ...
        <Form>
          ...
          <div className="col-md-6 mb-3">
          <Form.Group className="mb-3">
            <Form.Label className="text-light">{returnTitle('create_proj.currency')}</Form.Label>
            <Select
